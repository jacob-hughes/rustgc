...
struct NeedsDropTypes<'tcx, F> {
    ...
    /// need dropping we push them onto the stack.
...
fn adt_finalizer_tys(
    ...
    let adt_components = move |adt_def: ty::AdtDef<'tcx>, finalizer_optional: bool| {
        if adt_def.is_manually_drop() {
            ...
            debug!("adt_finalizer_tys: `{:?}` is a union", adt_def);
            return Ok(Vec::new().into_iter());
        }
    ...
    debug!("adt_finalizer_tys(`{}`) = `{:?}`", tcx.def_path_str(def_id), res);
    res.map(|components| tcx.intern_type_list(&components))
}

pub(crate) fn provide(providers: &mut ty::query::Providers) {
    *providers = ty::query::Providers { needs_finalizer_raw, adt_finalizer_tys, ..*providers };
}
