...
thread_local!{
    ...
    static LOCAL10: RefCell<Gc<Finalizable>> = RefCell::new(Gc::new(Finalizable(10)));
}

#[thread_local]
static mut LOCAL7: Option<Gc<Finalizable>> = None;
#[thread_local]
static mut LOCAL8: Option<Gc<Finalizable>> = None;
#[thread_local]
static mut LOCAL9: Option<Gc<Finalizable>> = None;
...
fn do_stuff_with_tls() {
    ...
    unsafe {
        LOCAL7 = Some(Gc::new(Finalizable(7)));
        LOCAL8 = Some(Gc::new(Finalizable(8)));
        LOCAL9 = Some(Gc::new(Finalizable(9)));
    }
    ...
    dynamic_value += unsafe { LOCAL7.unwrap().0 };
    dynamic_value += unsafe { LOCAL8.unwrap().0 };
    dynamic_value += unsafe { LOCAL9.unwrap().0 };

    LOCAL10.with(|l| {
        *l.borrow_mut() = Gc::new(Finalizable(2))
    });
...
fn main() {
    ...
    // assert_eq!(FINALIZER_COUNT.load(atomic::Ordering::Relaxed), 3);
}
